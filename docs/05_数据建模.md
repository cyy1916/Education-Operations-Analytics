...
- `operator_accounts(account_name, platform)` 唯一
- `posts(post_id)` 唯一
- `customers(platform_account)` 可做业务唯一键（允许空）
- `customer_student_rel(customer_id, student_id, status)` 需防重复
- `enrollment_students(enrollment_id, student_id)` 唯一
- `private_course_members(course_instance_id, student_id, status)` 需防重复
- `class_members(course_instance_id, student_id, status)` 需防重复
- `teacher_assignments(course_instance_id, teacher_id, start_date)` 唯一
- `academic_assignments(course_instance_id, employee_id, start_date)` 唯一
- `attendances(lesson_id, student_id)` 唯一

### 7.2 检查约束

- `teachers.timezone`、`students.timezone`、`course_schedules.schedule_timezone` 必须是合法 IANA 时区
- `teacher_availability.start_time < end_time`
- `student_availability.start_time < end_time`
- `lessons.start_at_utc < end_at_utc`
- `hours_transactions.change_type IN ('increase','decrease')`
- `hours_transactions.hours_changed > 0`
- `renewals.purchased_hours > 0`
- `enrollments.purchased_hours > 0`
